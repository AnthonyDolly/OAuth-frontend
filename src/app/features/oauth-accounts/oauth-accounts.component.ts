...
import { OAuthAccount } from '../../core/interfaces/user.interface';

@Component({
  selector: 'app-oauth-accounts',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './oauth-accounts.component.html',
  styleUrls: ['./oauth-accounts.component.css']
})
export class OAuthAccountsComponent implements OnInit {
  ...
  private loadAccounts(): void {
    this.loading = true;
    this.userService.listOAuthAccounts().subscribe({
      next: (accounts) => {
      ...
      error: (error) => {
        this.loading = false;
        this.error = 'Error cargando cuentas OAuth';
      }
    });
  }

  linkAccount(provider: 'google' | 'microsoft' | 'github' | 'linkedin'): void {
    this.authService.initiateOAuth(provider);
  }

  unlinkAccount(accountId: string): void {
    if (confirm('¿Estás seguro de que quieres desvincular esta cuenta?')) {
      this.userService.unlinkOAuthAccount(accountId).subscribe({
        next: () => {
          this.success = 'Cuenta desvinculada exitosamente';
          this.loadAccounts();
        },
        error: (error) => {
          this.error = error.error?.message || 'Error desvinculando cuenta';
        }
      });
    }
  }

  getProviderIconSvg(provider: string): string {
    switch (provider) {
      case 'google':
        return `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="width: 28px; height: 28px;">
